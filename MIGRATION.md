...
with the signing key at `$PREFIX/etc/apt/trusted.gpg.d/zero-termux.gpg`. Third-party sources (e.g. a leftover `termuxvoid.list`) are **not touched** — Zero-Termux only manages its own files.
