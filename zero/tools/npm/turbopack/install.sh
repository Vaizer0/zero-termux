...
LOG_FILE="$ZERO_CACHE/install_turbopack.log"
TURBO_DATA_DIR="$HOME/.local/share/zero-termux-data/node-glibc"
# Zero-Termux: justified pin — Node LTS 22.x for the glibc toolchain that
# turbopack's bundled binary requires; bump deliberately when upgrading the
# glibc toolchain (see scripts/maintenance/version-check).
...
_patch() {
	...
	local patch_script="$ZERO_PATH/tools/npm/turbopack/bin/patch-interp.py"
...
_install_wrappers() {
	local src="$ZERO_PATH/tools/npm/turbopack/bin"
