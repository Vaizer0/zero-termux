...
_compile_goose_helper_impl() {
  local HELPER_SRC="$ZERO_PATH/tools/ai/goose/helper/goose_helper.c"
...
_goose_create_proot_wrapper() {
  local wrapper_src="$ZERO_PATH/tools/ai/goose/bin/goose.proot"
...
_goose_create_ubuntu_wrapper() {
  ...
  local wrapper_src="$ZERO_PATH/tools/ai/goose/bin/goose"
  if [ ! -f "$wrapper_src" ]; then
    log_error "Wrapper template not found at $wrapper_src"
    return 1
  fi
