...
GENTLE_AI_DATA_DIR="${GENTLE_AI_DATA_DIR:-$HOME/.local/share/zero-termux-data/gentle-ai}"
...
_build_and_apply_patches_impl() {
  local patcher_src="$ZERO_PATH/tools/ai/gentle-ai/termux-patches.go"
  local patcher_bin="$ZERO_CACHE/termux-patcher"
...
install_gentle_ai() {
  ...
  mkdir -p "$(dirname "$LOG_FILE")" "$ZERO_CACHE"
