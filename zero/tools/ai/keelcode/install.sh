...
KEELCODE_DATA_DIR="$HOME/.local/share/zero-termux-data/keelcode"
...
_compile_keelcode_helper_impl() {
  local HELPER_SRC="$ZERO_PATH/tools/ai/keelcode/helper/keelcode_helper.c"
...
_keelcode_create_proot_wrapper() {
  local wrapper_src="$ZERO_PATH/tools/ai/keelcode/bin/keelcode.proot"
...
_keelcode_create_ubuntu_wrapper() {
  ...
  local wrapper_src="$ZERO_PATH/tools/ai/keelcode/bin/keelcode"
  if [ ! -f "$wrapper_src" ]; then
    log_error "Wrapper template not found at $wrapper_src"
    return 1
  fi
