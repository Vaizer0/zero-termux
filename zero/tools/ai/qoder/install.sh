...
_compile_qoder_helper_impl() {
  local HELPER_SRC="$ZERO_PATH/tools/ai/qoder/helper/qoder_helper.c"
...
_qoder_create_proot_wrapper() {
  local wrapper_src="$ZERO_PATH/tools/ai/qoder/bin/qodercli.proot"
...
_qoder_create_ubuntu_wrapper() {
  ...
  local wrapper_src="$ZERO_PATH/tools/ai/qoder/bin/qodercli"
  if [ ! -f "$wrapper_src" ]; then
    log_error "Wrapper template not found at $wrapper_src"
    return 1
  fi
