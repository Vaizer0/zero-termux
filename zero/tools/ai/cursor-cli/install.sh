...
CURSOR_DATA_DIR="$HOME/.local/share/zero-termux-data/cursor"
...
_cursor_create_proot_wrapper() {
  local wrapper_src="$ZERO_PATH/tools/ai/cursor-cli/bin/cursor.proot"
...
_cursor_create_ubuntu_wrapper() {
  ...
  local wrapper_src="$ZERO_PATH/tools/ai/cursor-cli/bin/cursor"
  if [ ! -f "$wrapper_src" ]; then
    log_error "Wrapper template not found at $wrapper_src"
    return 1
  fi
