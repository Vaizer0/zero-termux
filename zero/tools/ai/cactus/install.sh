...
CACTUS_CLI_LOG_FILE="$ZERO_CACHE/install_ai.log"
CACTUS_CLI_DATA_DIR="$HOME/.local/share/zero-termux-data/cactus-cli"
...
_cactus_cli_install_pip_glibc() {
  log_info "This downloads the Cactus Engine wheel (~24 MB) plus zero deps (numpy, huggingface-hub, fastapi) into the glibc Python environment"
...
_cactus_cli_post_install_glibc() {
  local launcher_src="$ZERO_PATH/tools/ai/cactus/bin/cactus.launcher.py"
...
_cactus_cli_create_glibc_wrapper() {
  local wrapper_src="$ZERO_PATH/tools/ai/cactus/bin/cactus.glibc"
...
_cactus_cli_create_proot_wrapper() {
  local wrapper_src="$ZERO_PATH/tools/ai/cactus/bin/cactus.proot"
  if [ ! -f "$wrapper_src" ]; then
    log_error "Wrapper template not found at $wrapper_src"
    return 1
  fi
...
_cactus_cli_create_ubuntu_wrapper() {
  ...
  local wrapper_src="$ZERO_PATH/tools/ai/cactus/bin/cactus"
  if [ ! -f "$wrapper_src" ]; then
    log_error "Wrapper template not found at $wrapper_src"
    return 1
  fi
