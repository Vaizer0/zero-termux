...
: "${ZERO_CACHE:=$HOME/.cache/zero-termux}"
: "${ZERO_PATH:=$HOME/zero-termux/zero}"
: "${PREFIX:=/data/data/com.termux/files/usr}"

LOG_FILE="$ZERO_CACHE/install_ai.log"
DROID_DATA_DIR="$HOME/.local/share/zero-termux-data/droid-factory"
...
_compile_droid_helper_impl() {
  local HELPER_SRC="$ZERO_PATH/tools/ai/droid-factory/helper/droid_helper.c"
...
_droid_create_proot_wrapper() {
  local wrapper_src="$ZERO_PATH/tools/ai/droid-factory/bin/droid.proot"
...
_droid_create_ubuntu_wrapper() {
  ...
  local wrapper_src="$ZERO_PATH/tools/ai/droid-factory/bin/droid"
  if [ ! -f "$wrapper_src" ]; then
    log_error "Wrapper template not found at $wrapper_src"
    return 1
  fi
