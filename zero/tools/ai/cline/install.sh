...
CLINE_DATA_DIR="$HOME/.local/share/zero-termux-data/cline"
...
_cline_create_proot_wrapper() {
  local wrapper_src="$ZERO_PATH/tools/ai/cline/bin/cline.proot"
...
_cline_create_ubuntu_wrapper() {
  ...
  local wrapper_src="$ZERO_PATH/tools/ai/cline/bin/cline"
  if [ ! -f "$wrapper_src" ]; then
    log_error "Wrapper template not found at $wrapper_src"
    return 1
  fi
