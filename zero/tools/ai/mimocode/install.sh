...
_compile_mimocode_helper_impl() {
  local HELPER_SRC="$ZERO_PATH/tools/ai/mimocode/helper/mimocode_helper.c"
...
_mimocode_create_proot_wrapper() {
  local wrapper_src="$ZERO_PATH/tools/ai/mimocode/bin/mimo.proot"
...
_mimocode_create_ubuntu_wrapper() {
  ...
  local wrapper_src="$ZERO_PATH/tools/ai/mimocode/bin/mimo"
  if [ ! -f "$wrapper_src" ]; then
    log_error "Wrapper template not found at $wrapper_src"
    return 1
  fi
