...
```bash
zero install ai --mimocode
```

## Uninstall

```bash
zero uninstall ai --mimocode
```

## Update

```bash
zero update ai --mimocode
```

## Notes

- Native installation requires `glibc-repo`, `glibc`, `clang`, and other dependencies (installed automatically)
- The real binary is stored in `~/.local/share/zero-termux-data/mimocode/`
- A small C bootstrapper (`mimocode_helper.c`) handles ELF loading via the glibc dynamic linker
- Data directory: `~/.local/share/zero-termux-data/mimocode/`
