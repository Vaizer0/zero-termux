...
FREEBUFF_DATA_DIR="$HOME/.local/share/zero-termux-data/freebuff"
...
_compile_freebuff_helper_impl() {
  local HELPER_SRC="$ZERO_PATH/tools/ai/freebuff/helper/freebuff_helper.c"
...
_freebuff_create_proot_wrapper() {
  local wrapper_src="$ZERO_PATH/tools/ai/freebuff/bin/freebuff.proot"
...
_freebuff_create_ubuntu_wrapper() {
  ...
  local wrapper_src="$ZERO_PATH/tools/ai/freebuff/bin/freebuff"
  if [ ! -f "$wrapper_src" ]; then
    log_error "Wrapper template not found at $wrapper_src"
    return 1
  fi
