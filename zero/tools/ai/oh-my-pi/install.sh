...
OMP_DATA_DIR="$HOME/.local/share/zero-termux-data/oh-my-pi"
...
_compile_omp_helper_impl() {
  local HELPER_SRC="$ZERO_PATH/tools/ai/oh-my-pi/helper/omp_helper.c"
...
_omp_create_proot_wrapper() {
  local wrapper_src="$ZERO_PATH/tools/ai/oh-my-pi/bin/omp.proot"
...
_omp_create_ubuntu_wrapper() {
  ...
  local wrapper_src="$ZERO_PATH/tools/ai/oh-my-pi/bin/omp"
  if [ ! -f "$wrapper_src" ]; then
    log_error "Wrapper template not found at $wrapper_src"
    return 1
  fi
