...
_download_codegraph_impl() {
	...
	if ! tar -xzf $PREFIX/tmp/codegraph-linux-arm64.tar.gz -C "$ZERO_DATA" &>>"$LOG_FILE"; then
...
_write_codegraph_wrapper_impl() {
	local wrapper_src="$ZERO_PATH/tools/ai/codegraph/bin/codegraph"
...
_uninstall_codegraph_impl() {
	if rm -rf "$ZERO_DATA/codegraph-linux-arm64" && rm -f "$PREFIX/bin/codegraph" &>>"$LOG_FILE"; then
		return 0
	else
...
_update_codegraph_remove_impl() {
	if ! rm -rf "$ZERO_DATA/codegraph-linux-arm64" &>>"$LOG_FILE"; then
