...
AGY_DATA_DIR="$HOME/.local/share/zero-termux-data/antigravity-cli"
...
_antigravity_compile_helper_impl() {
  local HELPER_SRC="$ZERO_PATH/tools/ai/antigravity-cli/helper/agy_helper.c"
...
_antigravity_create_proot_wrapper() {
  local wrapper_src="$ZERO_PATH/tools/ai/antigravity-cli/bin/agy.proot"
...
_antigravity_create_ubuntu_wrapper() {
  ...
  local wrapper_src="$ZERO_PATH/tools/ai/antigravity-cli/bin/agy"
  if [ ! -f "$wrapper_src" ]; then
    log_error "Wrapper template not found at $wrapper_src"
    return 1
  fi
