...
```bash
zero uninstall ai --kilocode-cli
```

## Update

```bash
zero update ai --kilocode-cli
```

## Notes

- **Native mode** requires `glibc-repo`, `glibc`, `clang`, and other dependencies (installed automatically)
- The native binary is stored in `~/.local/share/zero-termux-data/kilocode/`
...
- Data directory: `~/.local/share/zero-termux-data/kilocode/`
