...
KILOCODE_DATA_DIR="$HOME/.local/share/zero-termux-data/kilocode"
...
_compile_kilocode_helper_impl() {
  local HELPER_SRC="$ZERO_PATH/tools/ai/kilocode-cli/helper/kilocode_helper.c"
...
_kilocode_create_proot_wrapper() {
  local wrapper_src="$ZERO_PATH/tools/ai/kilocode-cli/bin/kilocode.proot"
...
_kilocode_create_ubuntu_wrapper() {
  ...
  local wrapper_src="$ZERO_PATH/tools/ai/kilocode-cli/bin/kilocode"
  if [ ! -f "$wrapper_src" ]; then
    log_error "Wrapper template not found at $wrapper_src"
    return 1
  fi
