...
OPENCODE_DATA_DIR="$HOME/.local/share/zero-termux-data/opencode"
...
_compile_opencode_helper_impl() {
  local HELPER_SRC="$ZERO_PATH/tools/ai/opencode/helper/opencode_helper.c"
...
_opencode_create_proot_wrapper() {
  local wrapper_src="$ZERO_PATH/tools/ai/opencode/bin/opencode.proot"
...
_opencode_create_ubuntu_wrapper() {
  ...
  local wrapper_src="$ZERO_PATH/tools/ai/opencode/bin/opencode"
  if [ ! -f "$wrapper_src" ]; then
    log_error "Wrapper template not found at $wrapper_src"
    return 1
  fi
