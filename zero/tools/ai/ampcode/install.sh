...
AMP_DATA_DIR="$HOME/.local/share/zero-termux-data/ampcode"
...
_compile_amp_helper_impl() {
  local HELPER_SRC="$ZERO_PATH/tools/ai/ampcode/helper/amp_helper.c"
...
_amp_create_proot_wrapper() {
  local wrapper_src="$ZERO_PATH/tools/ai/ampcode/bin/amp.proot"
...
_amp_create_ubuntu_wrapper() {
  ...
  local wrapper_src="$ZERO_PATH/tools/ai/ampcode/bin/amp"
  if [ ! -f "$wrapper_src" ]; then
    log_error "Wrapper template not found at $wrapper_src"
    return 1
  fi
