...
KIMCHI_DATA_DIR="$HOME/.local/share/zero-termux-data/kimchi"
...
_compile_kimchi_helper_impl() {
  local HELPER_SRC="$ZERO_PATH/tools/ai/kimchi/helper/kimchi_helper.c"
...
_kimchi_create_proot_wrapper() {
  local wrapper_src="$ZERO_PATH/tools/ai/kimchi/bin/kimchi.proot"
...
_kimchi_create_ubuntu_wrapper() {
  ...
  local wrapper_src="$ZERO_PATH/tools/ai/kimchi/bin/kimchi"
  if [ ! -f "$wrapper_src" ]; then
    log_error "Wrapper template not found at $wrapper_src"
    return 1
  fi
