...
CACTUS_DATA_DIR="$HOME/.local/share/zero-termux-data/cactus-needle"
...
_cactus_create_glibc_wrapper() {
  local wrapper_src="$ZERO_PATH/tools/ai/cactus-needle/bin/needle.glibc"
...
_cactus_create_proot_wrapper() {
  local wrapper_src="$ZERO_PATH/tools/ai/cactus-needle/bin/needle.proot"
  if [ ! -f "$wrapper_src" ]; then
    log_error "Wrapper template not found at $wrapper_src"
    return 1
  fi
...
_cactus_create_ubuntu_wrapper() {
  ...
  local wrapper_src="$ZERO_PATH/tools/ai/cactus-needle/bin/needle"
  if [ ! -f "$wrapper_src" ]; then
    log_error "Wrapper template not found at $wrapper_src"
    return 1
  fi
