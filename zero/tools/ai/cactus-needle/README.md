...
```bash
zero uninstall ai --cactus-needle
zero update ai --cactus-needle
```

## Notes

- The install method is recorded in `~/.local/share/zero-termux-data/cactus-needle/.install-method`
- The JAX cache (`~/.cache/jax`) is shared with other JAX workloads and is not removed on uninstall
- Data directory: `~/.local/share/zero-termux-data/cactus-needle/`