...
LOG_FILE="$ZERO_CACHE/install_ai.log"
...
_clone_engram_repo_impl() {
  if ! git clone --quiet https://github.com/Gentleman-Programming/engram "$ZERO_DATA/engram" &>>"$LOG_FILE"; then
...
_build_engram_impl() {
  if ! go build -C "$ZERO_DATA/engram/cmd/engram" -o $PREFIX/bin/engram &>>"$LOG_FILE"; then
...
_uninstall_engram_impl() {
  if rm -rf "$ZERO_DATA/engram" && rm "$PREFIX/bin/engram" &>>"$LOG_FILE"; then
    return 0
  else
...
_update_engram_pull() {
  if ! git -C "$ZERO_DATA/engram" pull &>>"$LOG_FILE"; then
...
_update_engram_build() {
  ...
  go clean -C "$ZERO_DATA/engram" -cache &>>"$LOG_FILE"
  go mod tidy -C "$ZERO_DATA/engram" &>>"$LOG_FILE"
  go mod download -C "$ZERO_DATA/engram" &>>"$LOG_FILE"
