...
CLAUDE_DATA_DIR="$HOME/.local/share/zero-termux-data/claude"
...
_compile_claude_helper_impl() {
  local HELPER_SRC="$ZERO_PATH/tools/ai/claude-code/helper/claude_helper.c"
...
_claude_create_proot_wrapper() {
  local wrapper_src="$ZERO_PATH/tools/ai/claude-code/bin/claude.proot"
...
_claude_create_ubuntu_wrapper() {
  ...
  local wrapper_src="$ZERO_PATH/tools/ai/claude-code/bin/claude"
  if [ ! -f "$wrapper_src" ]; then
    log_error "Wrapper template not found at $wrapper_src"
    return 1
  fi
