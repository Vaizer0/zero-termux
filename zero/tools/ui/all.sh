...
install_all_ui_components() {
	...
	for tool in "${UI_COMPONENTS[@]}"; do
		...
		cursor)
			...
			;;
		banner)
			loading "Installing Zero-Termux Banner" install_banner
			case $? in 0) ((installed_count++));; 1) ((failed_count++));; esac
			;;
		esac
	done
...
uninstall_all_ui_components() {
	...
	for tool in "${UI_COMPONENTS[@]}"; do
		...
		cursor)
			...
			;;
		banner)
			loading "Uninstalling Zero-Termux Banner" uninstall_banner
			case $? in 0) ((uninstalled_count++));; 1) ((failed_count++));; esac
			;;
		esac
	done
...
reinstall_all_ui_components() {
  ...
  for tool in "${UI_COMPONENTS[@]}"; do
    ...
    cursor)
      ...
      ;;
    banner)
      loading "Reinstalling Zero-Termux Banner" reinstall_banner
      case $? in 0) ((reinstalled_count++));; 1) ((failed_count++));; esac
      ;;
    esac
  done
