...
BUN_DATA_DIR="$HOME/.local/share/zero-termux-data/bun"
...
_install_bun_binary_impl() {
  ...
  cat > "${bin_dir}/bun" <<-'WRAPPER_EOF'
		#!/data/data/com.termux/files/usr/bin/bash
		# bun wrapper for Termux/Android — generated by zero-termux installer
...
_install_bun_proot_wrappers() {
  ...
  local wrapper_src="$ZERO_PATH/tools/lang/bun/bin/bun"
  ...
  local wrapper_bunx_src="$ZERO_PATH/tools/lang/bun/bin/bunx"
...
_bun_setup_path_native() {
  ...
  echo "# Added by zero-termux bun installer" >>"$rc_file"
...
_ensure_bun() {
  ...
  local _log_dir
  _log_dir="$(dirname "$_saved_log" 2>/dev/null || echo "$ZERO_CACHE")"
