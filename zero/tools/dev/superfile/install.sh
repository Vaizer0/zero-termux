...
LOG_FILE="$ZERO_CACHE/install_dev.log"
...
_clone_superfile_repo_impl() {
  if ! git clone --quiet --depth 1 --branch "v$(_get_latest_superfile_version)" https://github.com/yorukot/superfile "$ZERO_DATA/superfile" &>>"$LOG_FILE"; then
...
_build_superfile_impl() {
  if ! go build -C "$ZERO_DATA/superfile" -o "$PREFIX/bin/spf" &>>"$LOG_FILE"; then
...
_uninstall_superfile_impl() {
  if rm -rf "$ZERO_DATA/superfile" && rm -f "$PREFIX/bin/spf" &>>"$LOG_FILE"; then
    return 0
  else
