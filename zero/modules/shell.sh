...
LOG_FILE="$ZERO_CACHE/install_shell.log"
...
setupPersistentSession() {
	...
	mkdir -p "$ZERO_CACHE" 2>/dev/null || mkdir -p ~/.cache/zero-termux

	echo "$HOME" > ~/.cache/zero-termux/last_dir
...
LAST_DIR_FILE="$HOME/.cache/zero-termux/last_dir"
SESSION_TIMESTAMP="$HOME/.cache/zero-termux/.session_time"
SESSION_TIMEOUT=5

save_dir() {
  mkdir -p ~/.cache/zero-termux 2>/dev/null
  pwd > "$LAST_DIR_FILE"
