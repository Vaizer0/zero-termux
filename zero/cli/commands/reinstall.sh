...
reinstall_main() {
  ...
  if [[ $# -eq 0 ]]; then
    ...
    box "Zero Reinstall"
    echo
    log_info "Usage: zero reinstall <target>"
    log_info "Usage: zero reinstall <target> --tool1 --tool2"
    echo
    ...
    echo
    list_item "zero reinstall ai --qwen-code --ollama"
    list_item "zero reinstall db --postgresql --sqlite"
  ...
  if [[ -z "$module_target" ]]; then
    ...
    echo "Run 'zero reinstall' to see available targets"
    return 1
  fi
