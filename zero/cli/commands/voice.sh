...
voice_help() {
	...
	echo
	printf "    ${D_CYAN}zero voice${D_NC}                   # Show this help\n"
	printf "    ${D_CYAN}zero voice opencode${D_NC}          # Capture → nvim → opencode\n"
	printf "    ${D_CYAN}zero voice qoder${D_NC}             # Capture → nvim → qoder\n"
	printf "    ${D_CYAN}zero voice claude-code${D_NC}       # Capture → nvim → claude -p\n"
	printf "    ${D_CYAN}zero voice text${D_NC}              # Capture → nvim → print to stdout\n"
	printf "    ${D_CYAN}zero voice !${D_NC}                 # Alias for 'text'\n"
	echo
	separator_section "Requirements"
	echo
	list_item "Termux:API package: ${D_CYAN}pkg install termux-api${D_NC}"
	list_item "Neovim for editing: ${D_CYAN}zero install editor${D_NC}"
	list_item "Termux:API app: ${D_BLUE}github.com/termux/termux-api${D_NC}"
	echo
}
...
voice_main() {
	...
	if ! command -v nvim &>/dev/null; then
		...
		list_item "Install the editor: ${D_CYAN}zero install editor${NC}"
		separator
		exit 1
	fi
