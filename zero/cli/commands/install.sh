...
install_main() {
  ...
  if [[ $# -eq 0 ]]; then
    ...
    echo
    list_item "zero install ai --qwen-code --ollama"
    list_item "zero install db --postgresql --sqlite"
    list_item "zero install dev --gh --fzf --jq"
  ...
  if [[ -z "$module_target" ]]; then
    ...
    echo "Run 'zero install' to see available targets"
    return 1
  fi
