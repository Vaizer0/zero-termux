...
readonly BRAIN_DIR="$ZERO_DATA/brain"
...
_brain_ensure() {
	if [[ ! -d "$BRAIN_DIR" ]]; then
		...
		list_item "Run: ${D_CYAN}zero brain init${D_NC}"
		return 1
	fi
}
...
_brain_editor() {
	...
	if ! command -v nvim &>/dev/null; then
		...
		list_item "Install it: ${D_CYAN}zero install editor${D_NC}"
		return 1
	fi
...
brain_help() {
	...
	echo
	printf "    ${D_CYAN}zero brain init${D_NC}              # Create local brain\n"
	printf "    ${D_CYAN}zero brain save${D_NC}              # Interactive save\n"
	printf "    ${D_CYAN}zero brain search react${D_NC}      # Search all memories\n"
	printf "    ${D_CYAN}zero brain ls${D_NC}                # List all categories\n"
	printf "    ${D_CYAN}zero brain ls react${D_NC}          # List react category\n"
	printf "    ${D_CYAN}zero brain edit${D_NC}              # Interactive edit\n"
	printf "    ${D_CYAN}zero brain edit slug-name${D_NC}    # Edit by slug\n"
	printf "    ${D_CYAN}zero brain delete${D_NC}            # Interactive delete\n"
	printf "    ${D_CYAN}zero brain reset${D_NC}             # Destroy local brain\n"
	printf "    ${D_CYAN}zero brain show slug-name${D_NC}    # View memory + relations\n"
	printf "    ${D_CYAN}zero brain graph${D_NC}             # Show connection map\n"
	printf "    ${D_CYAN}zero brain skill${D_NC}             # Create a skill from memories\n"
	printf "    ${D_CYAN}zero brain sync${D_NC}              # Sync with GitHub\n"
	echo
}
...
brain_init() {
	...
	if ! $gh_ok; then
		...
		list_item "Install gh for GitHub sync: ${D_CYAN}zero install dev${D_NC}"
		separator
		return 0
	fi
	...
	local repo_name="zero-termux-brain"
...
brain_ls() {
	...
	if [[ -n "$filter" ]]; then
		...
		if [[ -z "$dirs" ]]; then
			...
			echo
			list_item "Add one: ${D_CYAN}zero brain save${D_NC}"
			return 0
		fi
	fi
...
brain_sync() {
	...
	if [[ ! -d "$BRAIN_DIR/.git" ]]; then
		...
		list_item "Run: ${D_CYAN}zero brain init${D_NC} to set up Git"
		separator
		return 1
	fi
	...
	if ! git remote -v &>/dev/null; then
		...
		list_item "Set up GitHub sync with: ${D_CYAN}zero brain init${D_NC}"
		separator
		return 0
	fi
...
brain_skill() {
	...
	if [[ ${#all_files[@]} -eq 0 ]]; then
		...
		echo
		list_item "No memories yet"
		list_item "Create some: ${D_CYAN}zero brain save${D_NC}"
		separator
		return 0
	fi
...
brain_edit() {
	...
	if [[ -z "$editor" ]]; then
		...
		list_item "Install one: ${D_CYAN}zero install editor${D_NC}"
		return 1
	fi
...
brain_reset() {
	...
	list_item "Recreate with: ${D_CYAN}zero brain init${D_NC}"
	separator
}
...
brain_dashboard() {
	...
	echo
	list_item "Save: ${D_CYAN}zero brain save${D_NC}"
	list_item "Show: ${D_CYAN}zero brain show${D_NC}"
	list_item "Search: ${D_CYAN}zero brain search${D_NC}"
	list_item "Edit: ${D_CYAN}zero brain edit${D_NC}"
	list_item "Delete: ${D_CYAN}zero brain delete${D_NC}"
	list_item "Reset: ${D_CYAN}zero brain reset${D_NC}"
	list_item "Graph: ${D_CYAN}zero brain graph${D_NC}"
	list_item "Skill: ${D_CYAN}zero brain skill${D_NC}"
	list_item "Sync: ${D_CYAN}zero brain sync${D_NC}"
	echo
}
