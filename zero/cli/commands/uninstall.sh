...
uninstall_main() {
  ...
  if [[ $# -eq 0 ]]; then
    ...
    box "Zero Uninstall"
    echo
    log_info "Usage: zero uninstall <target>"
    log_info "Usage: zero uninstall <target> --tool1 --tool2"
    echo
    ...
    echo
    list_item "zero uninstall ai --qwen-code --ollama"
    list_item "zero uninstall db --postgresql --sqlite"
  ...
  if [[ -z "$module_target" ]]; then
    ...
    echo "Run 'zero uninstall' to see available targets"
    return 1
  fi
