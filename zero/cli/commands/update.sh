...
update_main() {
  ...
  if [[ $# -eq 0 ]]; then
    ...
    echo
    list_item "zero       - Update only Zero-Termux framework"
    ...
    echo
    list_item "zero update ai --qwen-code --ollama"
    list_item "zero update db --postgresql --sqlite"
    list_item "Run ${D_CYAN}zero list <target>${D_NC} to see all available tools"
    echo
    return
  fi
  ...
  if [[ -z "$module_target" ]]; then
    ...
    echo "Run 'zero update' to see available targets"
    return 1
  fi
...
# Actualizar Zero-Termux
update_zero() {
  separator
  box "◈ UPDATING ZERO-TERMUX ◈"
  separator
  echo

  if [[ -d "$ZERO_PATH/../.git" ]]; then
    loading "Updating Zero-Termux" _update_zero_repo
    local rc=$?

    echo
    if [[ $rc -eq 0 ]]; then
      log_success "Zero-Termux updated"
    elif [[ $rc -eq 2 ]]; then
      log_success "Zero-Termux is already up to date"
    else
      log_error "Failed to update Zero-Termux"
      log_info "Check your internet connection or run git pull manually"
    fi

    rm -f "$ZERO_CACHE/new_version" "$ZERO_CACHE/last_version_check"
  else
    log_warn "Not a git repository, cannot update"
    log_info "If you installed via curl, reinstall with:"
    echo "  curl -fsSL https://raw.githubusercontent.com/Vaizer0/zero-termux/main/install.sh | bash"
  fi
