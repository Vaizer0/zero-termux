...
PG_LOG="$ZERO_CACHE/postgresql.log"
PG_USER="postgres"

# Mostrar ayuda
pg_help() {
	echo
	box "Zero PostgreSQL Manager"
	echo
	log_info "Usage: zero pg <command> [options]"
	echo
	...
	echo
	printf "    ${D_CYAN}zero pg start${D_NC}              # Start PostgreSQL\n"
	printf "    ${D_CYAN}zero pg stop${D_NC}               # Stop PostgreSQL\n"
	printf "    ${D_CYAN}zero pg create mydb${D_NC}        # Create database 'mydb'\n"
	printf "    ${D_CYAN}zero pg shell${D_NC}              # Open psql shell\n"
	echo
}
...
check_pg_installed() {
	if ! command -v pg_ctl &>/dev/null; then
		...
		log_info "Run: ${D_CYAN}zero install db${NC}"
		return 1
	fi
	return 0
}
...
pg_init() {
	...
	if check_pg_initialized; then
		...
		list_item "Run: ${D_CYAN}zero pg start${NC}"
		echo
		return 0
	fi
	...
	if loading "Initializing database" _pg_init_db; then
		...
		echo
		log_info "Start PostgreSQL with: ${D_CYAN}zero pg start${NC}"
	else
...
pg_start() {
	...
	if loading "Starting PostgreSQL" _pg_start_server; then
		...
		log_warn "PostgreSQL may not be initialized. Run: zero pg init"
		return 1
	fi
...
pg_status() {
	...
	if [[ -n "$found_dir" ]]; then
		...
		echo
		list_item "Run: ${D_CYAN}zero pg init${NC}"
	fi
...
pg_create() {
	...
	if [[ -z "$db_name" ]]; then
		...
		log_info "Usage: zero pg create <database_name>"
		return 1
	fi
...
pg_drop() {
	...
	if [[ -z "$db_name" ]]; then
		log_error "Database name required"
		log_info "Usage: zero pg drop <database_name>"
		return 1
	fi
