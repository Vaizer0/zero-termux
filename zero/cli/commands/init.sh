...
LOG_FILE="$ZERO_CACHE/init_project.log"
...
init_help() {
	...
	echo
	printf "    ${D_CYAN}cd my-next-app && zero init next${NC}\n"
	printf "    ${D_CYAN}cd my-react-app && zero init react${NC}\n"
	printf "    ${D_CYAN}cd api && zero init express${NC}\n"
	printf "    ${D_CYAN}cd backend && zero init nest${NC}\n"
	echo
}
...
_check_turbopack_toolchain() {
	[[ -x "$HOME/.local/share/zero-termux-data/node-glibc/bin/node" ]] && return 0
	return 1
}
...
configure_next() {
	...
	if _check_turbopack_toolchain; then
		...
		log_info "Turbopack requires the glibc toolchain (zero install npm --turbopack)"
