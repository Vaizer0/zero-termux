...
list_main() {
  ...
  for arg in "$@"; do
    ...
    *)
      ...
      echo "Run 'zero list' to see available targets"
      ;;
    esac
  done
}
...
_list_lang() {
  ...
  echo
  log_info "Install specific: ${D_CYAN}zero install lang --nodejs --python${NC}"
  log_info "Install all: ${D_CYAN}zero install lang${NC}"
  echo
}
...
_list_db() {
  ...
  echo
  log_info "Install specific: ${D_CYAN}zero install db --postgresql --sqlite${NC}"
  log_info "Install all: ${D_CYAN}zero install db${NC}"
  echo
}
...
_list_ai() {
  ...
  echo
  log_info "Install specific: ${D_CYAN}zero install ai --opencode --engram${NC}"
  log_info "Install all: ${D_CYAN}zero install ai${NC}"
  echo
}
...
_list_editor() {
  ...
  echo
  log_info "Install specific: ${D_CYAN}zero install editor --neovim${NC}"
  log_info "Install all: ${D_CYAN}zero install editor${NC}"
  echo
}
...
_list_dev() {
  ...
  echo
  log_info "Install specific: ${D_CYAN}zero install dev --gh --fzf --jq${NC}"
  log_info "Install all: ${D_CYAN}zero install dev${NC}"
  echo
}
...
_list_npm() {
  ...
  echo
  log_info "Install specific: ${D_CYAN}zero install npm --typescript --prettier${NC}"
  log_info "Install all: ${D_CYAN}zero install npm${NC}"
  echo
}
...
_list_shell() {
  ...
  echo
  log_info "Install specific: ${D_CYAN}zero install shell --powerlevel10k --fzf-tab${NC}"
  log_info "Install all: ${D_CYAN}zero install shell${NC}"
  echo
}
...
_list_ui() {
  ...
  table_row "Startup Banner" "--banner" "$(_grep_config "$HOME/.zshrc" "# ===== Zero-Termux Banner =====" "$HOME/.bashrc")"
  table_end

  echo
  log_info "Install specific: ${D_CYAN}zero install ui --font --extra-keys${NC}"
  log_info "Install all: ${D_CYAN}zero install ui${NC}"
  echo
}
...
_list_auto() {
  ...
  echo
  log_info "Install specific: ${D_CYAN}zero install auto --n8n${NC}"
  log_info "Install all: ${D_CYAN}zero install auto${NC}"
  echo
}
...
# Check if extra-keys are configured by zero-termux
_check_extra_keys() {
