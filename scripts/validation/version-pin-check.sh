...
#   zero/tools/**/install.sh    — Node LTS, Go minimum floors
...
# version-reporting commands, the zero CLI's own version
OK_RE='api\.github\.com|@latest|\$\{?[A-Za-z_]+\}?|\$[A-Za-z_]+|ZERO_VERSION=|-v \$\(gem|gem list'

VIOLATIONS="$( { find packages zero -type f \
  \( -path '*/DEBIAN/*' -o -name 'install.sh' -o -name '*.sh' -o -name '*.py' -o -name 'requirements.txt' -o -name '*.toml' \) \
  -not -path '*/data/*' -print0 2>/dev/null; \
  printf '%s\0' \
    packages/termux-penv/data/data/com.termux/files/usr/share/termux-penv/termux-pacman32.sh \
    packages/termux-penv/data/data/com.termux/files/usr/share/termux-penv/termux-pacman64.sh \
    packages/spider/data/data/com.termux/files/usr/lib/spider/report/requirements.txt; } \
  | xargs -0 -r rg -nN --no-messages -e "$PIN_RE" 2>/dev/null \
  | rg -v -e "$OK_RE" \
  | rg -v -e "$JUST_RE" || true)"
